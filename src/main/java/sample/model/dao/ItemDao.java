...
public class ItemDao {
	...
	public Item select(int shopId, int itemId) throws SQLException {
		...
		String sql = "SELECT shop_id, item_id, item_name, item_describe, item_price, status FROM item"
	...
	public int insert(Item item) throws SQLException {
		...
		String sql = "INSERT INTO item(shop_id, item_id, item_name, item_describe, item_price, status)"
				+ " VALUES(?, ?, ?, ?, ?, ?)";
	...
	public void update(Item item) throws SQLException {
		...
		String sql = "UPDATE item set item_name = ?, item_describe = ?, item_price = ?, status = ?"
				+ " WHERE shop_id = ? AND item_id = ?";
		...
		try(PreparedStatement stmt = con.prepareStatement(sql)) {
			...
			stmt.setInt(4, item.getItemStatus());
